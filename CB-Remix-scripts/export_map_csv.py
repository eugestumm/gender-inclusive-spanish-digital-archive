#!/usr/bin/env python3
"""
CB-Remix-scripts/export_map_csv.py

Turns the raw "config-map" sheet — already read into a DataFrame — into
the CSV file the Jekyll site's map config actually uses
(_data/config-map.csv), applying the sheet-specific cleanup that raw
spreadsheet exports need:
  ...
  - Resolves each row's "field" value from a human-readable, language-name
    ...
    to this sheet.
  - Normalizes the "search" checkbox column to lowercase "true" (Google's
    ODS export represents a checked box as the literal number 1, not a
    real boolean — see normalize_boolean_columns() below).
...
def export_map_csv(map_df, output_path, config_df=None):
    """Clean the raw config-map DataFrame and write it to *output_path* as
    CSV.

    *map_df* is the raw "config-map" sheet, as read straight off the
    spreadsheet (no cleanup applied yet). Expected columns include:
        field, translate_id_map, lang, search
    *output_path* is a pathlib.Path (or str) for the CSV file to write,
    e.g. _data/config-map.csv — its parent directory is created if needed.
    ...
    *config_df* is the "config" tab (columns: category, content), used to
    ...
    and passes straight through. If a new translated field is added later
    (e.g. "title"), it isn't in FIELD_RENAME_RULES yet — it'll fall back
    to the same "-en"/"-pt" hyphen default (see resolve_field_name()),
    with a warning printed so you notice and can add an explicit rule if
    that default is wrong for it.
    ...
    if "field" in df.columns:
        ...
        after = len(df)
        print(f"[INFO] [config-map] Dropped {before - after} empty/formula rows (kept {after})")
    ...
    print(
        f"[INFO] [config-map] lang1 = {lang1_name!r} ({lang1_id!r}), "
    ...
    def resolve_field_name(raw_field: str) -> str:
        ...
        if lang_name == lang1_name:
            ...
            print(
                f"[WARN] [config-map] field {raw_field!r} has language "
        ...
        if rule is None:
            print(
                f"[WARN] [config-map] no explicit rename rule for base field "
