#!/usr/bin/env python3
"""
CB-Remix-scripts/export_search_csv.py

Turns the raw "config-search" sheet — already read into a DataFrame —
into the CSV file the Jekyll site's search config actually uses
(_data/config-search.csv), applying the sheet-specific cleanup that raw
spreadsheet exports need:
  ...
  - Resolves each row's "field" value from a human-readable, language-name
    ...
    to this sheet.
  - Normalizes the "index"/"display" checkbox columns to lowercase
...
def export_search_csv(search_df, output_path, config_df=None):
    """Clean the raw config-search DataFrame and write it to *output_path*
    as CSV.

    *search_df* is the raw "config-search" sheet, as read straight off the
    spreadsheet (no cleanup applied yet). Expected columns include:
        field, lang, index, display
    *output_path* is a pathlib.Path (or str) for the CSV file to write,
    e.g. _data/config-search.csv — its parent directory is created if
    needed.
    ...
    *config_df* is the "config" tab (columns: category, content), used to
    ...
    ="title-in-"&config!B6, producing a human-readable value such as
    "title-in-English" / "title-in-Portuguese". resolve_field_name()
    ...
    built against:
        ...
        location-en / location-pt         (same "-en"/"-pt" hyphen
                                            pattern, but NOTE: singular
                                            "location", unlike
                                            metadata-orchestrator's/
                                            config-browse's plural,
                                            underscore-separated
                                            "locations_en"/"locations_pt"
                                            — this sheet genuinely uses a
                                            different field name for the
                                            same underlying concept, so
                                            don't assume it matches the
                                            other config files)
    ...
    if "field" in df.columns:
        ...
        after = len(df)
        print(f"[INFO] [config-search] Dropped {before - after} empty/formula rows (kept {after})")
    ...
    print(
        f"[INFO] [config-search] lang1 = {lang1_name!r} ({lang1_id!r}), "
    ...
    FIELD_RENAME_RULES = {
        ...
        "location":    {"lang1": "{base}-{lang1_id}", "lang2": "{base}-{lang2_id}"},
    }
    ...
    def resolve_field_name(raw_field: str) -> str:
        ...
        if lang_name == lang1_name:
            ...
            print(
                f"[WARN] [config-search] field {raw_field!r} has language "
        ...
        if rule is None:
            print(
                f"[WARN] [config-search] no explicit rename rule for base field "
